...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // Parse motor speeds separated by a comma
    int separatorIndex = message.indexOf(',');

    if (separatorIndex != -1) {
      String rightSpeed = message.substring(0, separatorIndex);
      String leftSpeed = message.substring(separatorIndex + 1);

      // Convert to integers
      Vel_R = rightSpeed.toInt();
      Vel_L = leftSpeed.toInt();
    }
